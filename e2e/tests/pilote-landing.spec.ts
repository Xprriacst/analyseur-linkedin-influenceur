 ...
 *    avant inscription.
 * 5. Pas de bouton Google (OAuth inexistant = bouton mort).
 * 6. Mots de passe différents : aucun compte créé.
 * 7. Après un signup avec session, le lien d'invitation n'apparaît que si
 *    `GET /pilote/invite` renvoie une URL https.
 */
...
test.describe("Landing /pilote", () => {
  test("compte la vue, affiche les verbatims et le groupe, sans Google ni lien Skool", async ({
    page,
  }) => {
    let views = 0;
    ...
    await expect(page.getByText("Continuer avec Google")).toHaveCount(0);
