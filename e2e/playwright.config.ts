...
export default defineConfig({
  ...
  projects: [
    { name: "setup", testMatch: /auth\.setup\.ts/ },
    {
      ...
      // (backend mocké dans le spec — aucun login, aucun coût).
      name: "public",
      testMatch: /smoke\.spec\.ts|full-audit-funnel\.spec\.ts/,
      use: { ...devices["Desktop Chrome"] },
    },
    {
      name: "authenticated",
      testIgnore: [/smoke\.spec\.ts/, /full-audit-funnel\.spec\.ts/, /cross-user-isolation\.spec\.ts/],
      dependencies: ["setup"],
      use: {
