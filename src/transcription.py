...
Le téléchargement (garde-fou SSRF) est mutualisé dans `net_guard`.
"""
...
def _download(url: str) -> tuple[str, bytes]:
    ...
    filename, data, _content_type = guarded_download(
        url,
        allowed_exts=_AUDIO_EXTS,
        default_ext="mp3",
        max_bytes=_MAX_AUDIO_BYTES,
        allowed_hosts_env="IG_AUDIO_ALLOWED_HOSTS",
        error_cls=TranscriptionError,
        filename_stem="audio",
        user_agent="lkd-outreach/ig-agent",
    )
